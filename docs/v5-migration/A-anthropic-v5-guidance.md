...
**Path-scoped rules also exist at user scope — `~/.claude/rules/`, not only the project-scoped location.** Stage 1 unit 1.0 (`.scratchpad/S1-unit-1.0-pathscoped.md`) verified this by a direct fetch of the same memory page and found a "User-level rules" subsection none of Document A's original six passes ever quoted:
...
Source: https://code.claude.com/docs/en/memory — "User-level rules". The page states the `paths:`-gating capability as a property of the `.claude/rules/` mechanism as a whole, before branching into project- and user-scope variants — nowhere does it restrict `paths:` gating to project scope. A separate warning box on the same page independently corroborates `~/.claude/rules/` as a first-class user-scope memory location, grouped with `~/.claude/CLAUDE.md`: "Imports in user-scope memory files, such as `~/.claude/CLAUDE.md` and `~/.claude/rules/`, are files you wrote yourself, so they load without the dialog and carry the same trust as the rest of your personal configuration." This closes a real gap in Document A's original coverage: the earlier passes recorded path-scoped rules only via their project-scoped citations, understating the mechanism set for exactly the artifact class — a user-scope CLAUDE.md — that a context-reduction migration most needs to shrink.

**Whether this user-scope gating survives into a non-fork sub-agent is NOT DOCUMENTED.** Anthropic's sub-agent startup enumeration (https://code.claude.com/docs/en/sub-agents — "What loads at startup") states:

> A non-fork subagent's initial context contains: ... **CLAUDE.md files**: every level of the CLAUDE.md hierarchy the main conversation loads, including `~/.claude/CLAUDE.md`, project rules, `CLAUDE.local.md`, and managed policy files.

The enumeration names "project rules," not user-level rules, and "the main conversation loads" describes a frozen snapshot of what has already loaded by delegation time — it does not state whether a subagent's own subsequent file reads independently re-trigger `paths:`-gated loading inside the subagent's own execution. No page in the corpus (`memory`, `sub-agents`, or the `InstructionsLoaded` hook page) resolves this either direction. This is recorded in the same honest register this document already uses for its silence on emphasis technique (see [§ Where The Sources Are Silent](#where-the-sources-are-silent)): a documented absence, not a probable yes. See [§ Coverage Gaps](#coverage-gaps) for open-item tracking and an adjacent non-official caveat on runtime behavior.
...
### Gap Closed, And Two Opened, By The Unit 1.0 Finding

**Closed:** Document A's original six passes, and the A7 amendment, recorded path-scoped rules only via project-scoped citations and never quoted the memory page's "User-level rules" subsection — understating the mechanism set for exactly the artifact class, a user-scope CLAUDE.md, that a context-reduction migration most needs to shrink. Stage 1 unit 1.0 (`.scratchpad/S1-unit-1.0-pathscoped.md`) closed this by a direct fetch of the same official page; recorded in [§ Splitting Content Across Files](#splitting-content-across-files--which-mechanisms-reduce-context-and-which-do-not).

**Open — sub-agent gating survival is NOT DOCUMENTED.** Whether a non-fork sub-agent's own file reads re-trigger `paths:`-gated rule loading independently of what the parent conversation had already loaded is not addressed by any official source found (`code.claude.com/docs/en/memory`, `code.claude.com/docs/en/sub-agents`, `code.claude.com/docs/en/hooks#instructionsloaded`). No later unit may treat "relocate content into a user-scope path-scoped rule" as a settled route to context reduction inside sub-agent-delegated work until this is resolved.

**Open, and explicitly non-official — a third-party report that the mechanism may not work at all in user scope.** GitHub issue `anthropics/claude-code#21858` (closed, labeled `stale`) reports that as of 2026-01-30, `paths:` frontmatter placed in `~/.claude/rules/` was not respected in practice — the rule never loaded regardless of file match, while the identical file worked when placed in the project-scoped `./.claude/rules/`. **This is an unverified, third-party GitHub issue report, not an Anthropic statement of intended behavior, and it must not be read as Anthropic guidance or entered into the Directive Catalog.** It is recorded here, in Coverage Gaps, precisely because it is the reason the plan cannot yet rely on the user-scope mechanism even though the mechanism's existence resolves YES: the specification and the current runtime behavior are not shown to agree. Whether this defect is still live against the current product version is itself unverified — the issue's label state was checked directly, but no changelog or release notes were fetched to confirm a fix.
...
### Amendment 2 — 2026-07-27 — user-scope path-scoped rules gap closed by unit 1.0

**Driven by:** `.scratchpad/S1-unit-1.0-pathscoped.md`, a read-only meta-review card that verified two Anthropic pages directly rather than relying on Document A's prior six-and-a-half passes. Commissioned because Stage 1 unit 1.0 found that Document A's coverage of path-scoped rules cited only the project-scoped location and never quoted the memory page's "User-level rules" subsection — a gap that mattered because the file this migration most needs to shrink is user-scope.

**Added:**

- A user-scope `~/.claude/rules/` finding in [§ Splitting Content Across Files](#splitting-content-across-files--which-mechanisms-reduce-context-and-which-do-not), quoting the "User-level rules" subsection and the user-scope-imports warning box at https://code.claude.com/docs/en/memory.
- A documented-absence finding, in the same section, on whether `paths:` gating survives into a non-fork sub-agent — recorded as NOT DOCUMENTED, since the sub-agents page's startup enumeration names "project rules," not user rules, and is silent on whether a subagent's own file reads re-trigger gating independently.
- A new [§ Gap Closed, And Two Opened, By The Unit 1.0 Finding](#gap-closed-and-two-opened-by-the-unit-10-finding) subsection under [§ Coverage Gaps](#coverage-gaps), tracking the closed gap and the two that remain open: the sub-agent gating question, and whether a third-party-reported runtime defect (`anthropics/claude-code#21858`) affecting `paths:` gating in `~/.claude/rules/` is still live.

**What the amendment did not change:** No existing finding was weakened — in particular the emphasis-silence finding ([§ Emphasis And Over-Steering](#emphasis-and-over-steering)) and the `@path`-imports scope limit ([D37](#d37--do-not-use-path-imports-to-reduce-context-cost-claudemd)) stand exactly as before. The third-party GitHub issue is recorded strictly as a non-official, unverified caveat in Coverage Gaps, not as a directive and not as Anthropic guidance — Document A's citation base remains official-Anthropic-sources-only throughout. No section was restructured and no content was reduced; this amendment is additive only.
