...
| Check-in 5-field template behind a discretionary read for the staff tier — decide whether to inline | Stage 2 | ✅ **DECIDED: do not inline. No action needed; this item is closed, not pending.** `staff-engineer.md` is fully injected, so every line costs context on every session — and the template requires an interactive user, which makes it inapplicable to the background sub-agents that do the actual work. Inlining would spend context on every session to serve a case the injected tier cannot reach. The discretionary read stays. |
...
| `manage-pr-comments` `Bash(gh *)` deny-override (Q10) | Stage 4 | ✅ **IMPLEMENTED** (card #3101). The blanket `Bash(gh *)` is replaced by `Bash(gh auth status *)`, `Bash(gh pr view *)`, `Bash(gh api *)` — every `gh` invocation the skill documents, mapped 1:1, none uncovered. **The mechanism first proposed for this did not exist:** `perm` has no deny or block subcommand (it manages `permissions.allow` only), and hand-editing a settings file is prohibited. So it was fixed at the source instead — a skill's `allowed-tools` is a grant list and cannot express a deny, so you narrow it by granting less, which is the better fix regardless. Verb-family wildcards (`Bash(gh pr *)`, `Bash(gh auth *)`) stay excluded because they reach `gh pr merge`, `gh pr close`, and `gh auth logout`; a trailing wildcard on a single read-only verb is not the same thing. Trailing ` *` was confirmed against Anthropic's documentation to enforce a word boundary, so bare invocations still match — an exact-match pattern would have silently auto-denied `gh pr view --json` in `dontAsk` mode and stalled a background agent with no signal. |
...
| Document D's protected-content table is keyed to the pre-edit 530-line global file — re-derive before any further edit to that file | whoever edits it next | ✅ **Re-derived, and the whole citation class closed with it** (cards #3102 → #3103 → #3104 → #3105, Amendments 9–12). All 15 protected items were located intact in the current 450-line file, re-summing to exactly 183 — **nothing was lost in the migration.** The sweep then found the same defect elsewhere: 95 citation strings across ~35 files, of which 15 were re-keyed, 9+ verified unchanged, and the rest correctly classified *historical* and deliberately left — re-keying a citation inside an amendment falsifies the audit trail. All 29 output-style citations were then verified individually; git archaeology recovered the original sentence for the four unverifiable ones, byte-identical and merely shifted, and they are now **content-anchored** so they cannot rot again. `senior-staff-engineer.md:47-49` came back **present, unmoved, unchanged** — it was never review-protocol content, so no validation gate lost its target. Standing recommendation on the remaining 19: **do not re-key them** — the migration is closed, both files churn regardless of citation format, and `senior-staff-engineer.md` disclaims line-number cross-references anyway. |

**Two were already fixed, one is decided with no action needed, and four were real work now done.** The item that had been dismissed longest — "out of migration scope, one-line fix whenever" — turned out to be shipped documentation describing a deleted subsystem in the generated tool catalog. The one marked DO NOT RE-LITIGATE as owner decision Q10 had never been implemented at all, and the mechanism first proposed for it did not exist.

**On the two items previously written up here as "recommendations, owner's call."** That framing was wrong and has been removed. One was a decision this session could simply make on unambiguous grounds, and did. The other was a decision the owner had *already made* at Stage 1 and nobody had executed — recording a recommendation to do the thing that was already decided is not a hand-off, it is a way of not doing it.
