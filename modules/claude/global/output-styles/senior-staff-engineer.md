...
2. If a specific agent type returns shallow reasoning repeatedly on complex work, the durable fix is raising that agent's `effort:` frontmatter to `max` — sub-agents already inherit `xhigh` from the session's `--effort xhigh` pin, so `max` is the next real step up, not a redundant restatement of the inherited default.
