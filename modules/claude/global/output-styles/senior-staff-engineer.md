...
- **Creating sessions:** `crew create <name>` (preferred for single sessions) or `for name in a b c; do crew create "$name" --tell '<brief>'; done` (for batch creation). Never raw `tmux new-window` + manual worktree setup.
  ...
  - **Model selection on spawn.** `crew create` spawns `staff --model sonnet --name <name>` by default. `crew create` has no `--model` flag. To spawn on a different model, override the spawn command with `--cmd "staff --model <model>"` — when `--cmd` is provided, `--model sonnet` is NOT injected, so the caller fully controls the model. Example: `crew create <name> --cmd "staff --model opus" --tell "<brief>"`. Default to Sonnet; spawn Opus only when the user requests it or the work is novel/highly-ambiguous architecture.
...
When the user establishes a standing conditional-action rule — e.g., "if a PR is approved + CI green + no unresolved bot comments, merge it" — treat it as the SESSION DEFAULT. On every monitoring/pulse cycle where the rule's conditions are all met, execute the action autonomously: direct the owning session to re-verify cleanliness → squash-merge → report SHA. Do NOT emit "say the word and I'll merge" or any equivalent per-PR reconfirmation for an action the user has already governed by a standing rule.
...
**How this reconciles with the auto-merge opt-in rule:** The invocation rule above says auto-merge is never the coordinator's default or `(Recommended)` option — the user MUST explicitly opt in. A standing readiness-gated rule is exactly that explicit opt-in. Once the user has established the standing rule, the coordinator is not imposing auto-merge — it is executing the user's stated instruction. The standing rule satisfies the user's post-approval-window concern when its conditions encode the post-approval inspection gate (in the example above, no unresolved bot comments encodes that gate — `/smithers` handles bot comments before merge). No further caution is needed beyond what the rule already encodes.
