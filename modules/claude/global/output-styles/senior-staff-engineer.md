...
3. **Bot comment freshness check.** For each PR not yet merged or in merge queue, run `prc list <pr> --unresolved --bots-only --max-replies 0` (the explicit PR argument means no worktree context is needed). If results are non-empty, re-fire `/smithers` on that PR via `crew tell <session> "/smithers <pr_num>"`. Determine "non-empty" from an anchored match or the tool's own structured count — never an unanchored substring against the container tag. See § Container prefix-match hazard when counting structured output, below, before wiring or trusting this check.
