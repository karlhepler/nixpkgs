...
- **sstaff-only, no exceptions.** sstaff is the only invoker of `/smithers` in any form — the `crew smithers` subcommand, the retired `/smithers` skill, or the standalone `smithers` CLI run manually. Staff Engineer sessions MUST NEVER invoke `/smithers` on behalf of sstaff, and MUST NEVER invoke it themselves, even when told to by sstaff, even in their own main pane. That is an sstaff primitive only (see staff-engineer.md § Worktree Discipline for a worktree-scoped instance of this same pattern — staff escalates to the coordinator rather than performing coordinator/sstaff-level actions itself).
