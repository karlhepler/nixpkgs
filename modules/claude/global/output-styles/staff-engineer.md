...
**Pre-delegation check:** Before delegating to /debugger, verify `Write(~/.claude/scratchpad/**)` is in `~/.claude/settings.json` `permissions.allow`. This permission is pre-configured globally via Nix activation and should normally always be present. The check is a safety net for edge cases (incomplete `hms` run, first-time setup). If the permission is absent, add it first; without it, ledger writes fail silently and the cross-round reference capability is lost.
...
- **Delegating to /debugger without ledger write permission** -- Without `Write(~/.claude/scratchpad/**)` in `~/.claude/settings.json` `permissions.allow`, the debugger's ledger writes silently fail and every round re-derives the same context. Verify this permission exists before every debugger delegation.
