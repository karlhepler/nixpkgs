...
**Routine-repeat warning:** Run this restatement even when the Write feels like a routine repeat of a path used earlier in the session — typos creep in via auto-completion, muscle memory, or token-similar paths (e.g., `cl/` vs `mctx/`), and routine-feeling repeats are precisely when verification gets skipped.
...
**Anti-pattern (routine-repeat failure):** A coordinator mid-task wrote a scratchpad file to `/Users/karlhepler/github.com/mctx-ai/cl/.scratchpad/` instead of `/Users/karlhepler/github.com/mctx-ai/mctx/.scratchpad/` — the same filename pattern as several prior writes that session, but with a typoed parent directory (`cl/` instead of `mctx/`). The routine-repeat feeling bypassed the reflex entirely. Familiarity is when this rule fails, not when it stops applying.
...
- **criteria** -- typically 3-5 total, including any mandatory standard ACs on review/research cards (specific, measurable outcomes)
  ...
  **Banned MoV patterns (pre-creation audit):**
  ...
  - ❌ Case-insensitive identifier matching with `.` wildcard against design-required lifecycle terms — when asserting absence of an agent identifier (e.g., `ac-reviewer`, `swe-foo`), prefer case-sensitive literal match (`! rg -q 'ac-reviewer'`) or fixed-strings (`! rg -qF 'ac-reviewer'`). Case-insensitive (`-i`) plus regex wildcards (`.`) collide with the lifecycle term that motivates the agent name. Worked example: `! rg -qi 'ac.reviewer' file` matches BOTH `ac-reviewer` (hyphenated agent identifier — the actual target) AND `AC reviewer` (lifecycle concept used throughout prompt files describing the SubagentStop hook workflow). The MoV passes only when the prompt is damaged. Lifecycle prose is design-required; the identifier is the artifact. The MoV must distinguish them. See § Card Sizing and Scope — pre-creation checklist (MoV mental dry-run, step 4 — Identifier collision) for the general rule.
