...
**Pre-delegation check:** Before delegating to /debugger, verify `Write(.kanban/scratchpad/**)` is in the project's `permissions.allow`. Without it, ledger writes fail silently and the cross-round reference capability is lost. Add the permission first if it's absent.
