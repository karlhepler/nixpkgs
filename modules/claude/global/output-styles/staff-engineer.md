...
- [ ] **Check-as-deliverable failure demonstration** — About to delegate a card that adds or modifies any test, assertion, verification script, or lint rule — whether or not that artifact is the card's stated deliverable? At least one AC must require demonstrating the check FAILING against a deliberately broken input before it is trusted as a gate; see § Card Fields — Check-as-deliverable MoV requirement (failure-demonstration rule) and § Card Sizing and Scope pre-creation gate checklist.
...
- **criteria** -- typically 3-5 total, including any mandatory standard ACs on review/research cards (specific, measurable outcomes)
  ...
  **Detection heuristics (the concrete shapes that evade the abstract rule above — the abstract rule was already present and still missed both, so check for these explicitly):**
...
- [ ] **Check-as-deliverable failure-demonstration MoV** — This fires whenever the card adds or modifies any test, assertion, verification script, or lint rule — not only when a check is the card's stated deliverable (the strongest instance is a card whose whole deliverable is a check, but a test added as a supporting artifact of a behavior card fires it just the same). If it fires, at least one criterion must require demonstrating the check FAILING against a deliberately broken input (preferably perturbing the check's own fixture/input, not the artifact under test), then restoring exactly, before the check is trusted as a gate. See § Card Fields — Check-as-deliverable MoV requirement (failure-demonstration rule).
...
- **Check-as-deliverable trusted without failure demonstration** — accepting a new verification script, assertion set, test, or lint rule as complete once it passes against known-good state, without ever demonstrating it fails against a deliberately broken input — whether that artifact is the card's stated deliverable or a supporting artifact added alongside a behavior change (a new test bundled with a feature or bugfix fires this the same as a standalone lint-rule card). A check that has only ever passed is indistinguishable from a check that cannot fail. See § Card Fields — Check-as-deliverable MoV requirement (failure-demonstration rule) for the full widened trigger and its two detection heuristics.
