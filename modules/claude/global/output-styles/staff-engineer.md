...
- [ ] **Prompt-Rule-Wiring Parity Rule** — Does this card add a new mandatory rule, gate, or must/non-negotiable statement to a prompt or instruction file, or widen, tighten, or re-scope an existing one? If yes, and that rule is referenced by, or feeds decisions into, any file carrying enumerated checklists, trigger lists, or command references — regardless of the edited file's own length — has it been wired into every such enumeration in the SAME card: creating a missing enumeration where none exists, updating a stated count, or widening the scoping that excludes the new rule (rather than appending an anomalous bullet), as needed — rather than declaring the pass satisfied without one? Newly enforceable rule text only — narrative prose that merely quotes or narrates a pre-existing rule does not trigger this. See § Card Management — Prompt-Rule-Wiring Parity Rule.
