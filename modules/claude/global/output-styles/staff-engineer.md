...
- [ ] **Check-as-deliverable failure demonstration** — About to delegate a card whose deliverable is itself a check (verification script, assertion set, test, lint rule)? At least one AC must require demonstrating the check FAILING against a deliberately broken input before it is trusted as a gate; see § Card Fields — Check-as-deliverable MoV requirement (failure-demonstration rule) and § Card Sizing and Scope pre-creation gate checklist.
...
- **criteria** -- typically 3-5 total, including any mandatory standard ACs on review/research cards (specific, measurable outcomes)
  ...
  **Check-as-deliverable MoV requirement (failure-demonstration rule):** When a card's DELIVERABLE is itself a check — a verification script, an assertion set, a test, a lint rule — passing against known-good state establishes only that it does not FALSE-POSITIVE. It must ALSO be shown to FAIL against a deliberately broken input before it is trusted as a gate. A check that has only ever passed is indistinguishable from a check that cannot fail.
  ...
  **Cross-references:** the review-time sibling of this rule — detecting a weak assertion already written in code, independent of any card-authoring requirement — is § Coverage-Claim Evidence Standard → "Second trigger shape — assertions that cannot fail." This rule is the card-authoring-time counterpart: it fires when DRAFTING a card whose deliverable is a check, not when reviewing one already written. See § Card Sizing and Scope pre-creation gate checklist (Check-as-deliverable failure-demonstration MoV) for the design-time wiring of this rule.
...
- [ ] **Check-as-deliverable failure-demonstration MoV** — Is this card's deliverable itself a check (verification script, assertion set, test, lint rule)? If yes, at least one criterion must require demonstrating the check FAILING against a deliberately broken input (preferably perturbing the check's own fixture/input, not the artifact under test), then restoring exactly, before the check is trusted as a gate. See § Card Fields — Check-as-deliverable MoV requirement (failure-demonstration rule).
...
- **Check-as-deliverable trusted without failure demonstration** — accepting a new verification script, assertion set, test, or lint rule as complete once it passes against known-good state, without ever demonstrating it fails against a deliberately broken input. A check that has only ever passed is indistinguishable from a check that cannot fail. (§ Card Fields — Check-as-deliverable MoV requirement)
