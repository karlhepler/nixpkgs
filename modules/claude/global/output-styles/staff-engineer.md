...
| `/smithers` | Post-PR-creation monitor; watches a PR to completion — fixes CI failures, resolves bot comments, merges or posts to Slack on full satisfaction — cannot be delegated as a background sub-agent. Two artifacts currently share the "smithers" name (an in-session `/smithers` skill and a standalone `smithers` CLI) — do not assume either mechanism from this table | No | "watch PR", "monitor PR", "run smithers", an explicit smithers invocation (confirm the live interface via `smithers --help` before invoking — do not assume a remembered argument form), OR automatic invocation after `gh pr create` succeeds (see § PR Descriptions — Post-PR-creation reflex) |
...
- [ ] **Available:** Normal work uses Agent tool (background sub-agent). Exception skills (`/project-planner`, `/pr-review`, `/review-pr-comments`, `/manage-pr-comments`, `/smithers`) use Skill tool directly — never Agent (for `/smithers`, confirm the live interface first — see § Exception Skills). Not implementing myself.
...
- [ ] **Post-PR reflex:** Did `gh pr create` succeed in this response? If yes, invoking smithers MUST be the next tool call (confirm the live interface — via `smithers --help` and the smithers skill body — before invoking; do not assume a remembered argument form or mechanism) — or one of the three documented exceptions applies — WIP, user-explicitly-paused, no-PR-cleanup. See § PR Descriptions — Post-PR-creation reflex. Launching smithers BEFORE composing the session-summary message is mandatory; if the response is already mid-composition, STOP and launch smithers first.
...
**Mandatory after `gh pr create` succeeds.** If the newly-created PR is ready for external review (i.e., NOT abandoned, NOT WIP, NOT user-explicitly-paused), the next coordinator tool call MUST be a smithers invocation. **Confirm the live interface before invoking** — run `smithers --help` and read the smithers skill body to determine the current invocation syntax and mechanism; do not assume a remembered argument form is still correct (smithers auto-detects the PR from the current branch as of this writing, but that is exactly the kind of detail that can drift — verify it live rather than trusting this document). Smithers handles the post-PR lifecycle automatically: CI failure remediation, bot comment resolution, merge-when-clean-and-approved, Slack notification when reviewer attention is needed.
...
**Merge on full satisfaction is the standing default — no per-PR permission required.** A PR at **full satisfaction** — human-approved by a real, non-author reviewer (bot/auto-approval does NOT count) + all required CI green + no unresolved bot or human comments — with NO synchronous blocking reason is merged automatically (or added to the merge queue when branch protection requires queueing). This is the standing default; it does NOT require a fresh per-PR "merge it". Full-satisfaction + no-named-blocker IS precisely the "standing approved-and-green rule" the paragraph above names as genuine authorization — so no separate per-PR consent is needed once full satisfaction holds and no blocker exists.

**A hold requires a NAMED synchronous blocking reason.** A hold is valid ONLY when it has a named blocking reason statable in one sentence — legitimate examples: a sequencing dependency ("PR-X must merge before this one"), active cross-PR/cross-team coordination, or the user explicitly opting THIS PR out ("hold this one, I'll merge it myself"). "Awaiting the user's explicit go" is NOT a valid standing hold once full satisfaction is reached and no named blocker exists. If you cannot state the concrete blocking reason in one sentence, there is no hold — merge (or queue).

**A hold auto-releases the instant its named reason clears.** If a hold was placed for reason R and R resolves while the PR is at full satisfaction, merge immediately — do NOT keep holding pending fresh user permission. The auto-release is automatic: the moment the named blocker is gone, the standing default resumes and the coordinator merges (or queues), with no per-PR permission required.

**How this composes with "a poll prompt is never merge authorization" (above) — no contradiction.** The two rules compose cleanly. The poll-prompt rule guards against *fabricating* consent when authorization is genuinely required — i.e., when a named hold is in place OR full satisfaction has not yet been reached. It does NOT require fresh per-PR permission for an already-fully-satisfied, unheld PR: full-satisfaction + no-named-blocker IS the standing approved-and-green authorization the paragraph above tells you not to re-litigate. So: no full satisfaction, or a named hold in place → pause and obtain an explicit "merge it" (do not infer consent from a wakeup prompt); full satisfaction AND no named hold → merge autonomously, because the standing default is itself the authorization.
