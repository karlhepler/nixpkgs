...
**Silent-stop diagnostic (no `<result>` block):** when a task notification carries no `<result>` block at all — only usage stats — interrogate the transcript before hypothesizing. Guessing at a cause and restructuring work around each guess is expensive: one incident guessed three times in sequence ("card too large," then "over-reading," then "unregistered `gh` permission"), costing ~4 wasted agent runs, one unnecessary cancellation, and one unnecessary split, before a two-command diagnostic finally found the real cause in a completely different hook. The Agent tool result names an `output_file` holding the full sub-agent JSONL transcript. That file must NEVER be read or tailed in full (`cat`, `head`, `tail`, or an unbounded `rg`) — it will overflow context — but it CAN be safely *searched*, because a count query and a bounded extraction each return only a few characters.

Step 1, signature counts:
```
for p in 'permission' 'denied' 'not allowed' 'Error' 'timed out' 'max.turns'; do
  printf '%-14s %s\n' "$p" "$(rg -ci "$p" "$OUTPUT_FILE" 2>/dev/null || echo 0)"
done
```
Step 2, bounded extraction of whichever signature is non-zero:
```
rg -o -m 2 '.{0,100}denied.{0,160}' "$OUTPUT_FILE"
```
(Use `rg`'s own `-m 2` max-count flag rather than piping into `head` — a pipeline whose final stage is `head`/`tail`/`cut`/`sort`/`tr` is itself denied by `kanban-mov-lint-hook.py`, so a `| head -2` suffix on this exact diagnostic would be denied if run literally.) Two commands beat three hypotheses. **`perm check` is not usable as evidence here** — it returns a non-zero exit for commands that demonstrably work, so a failing `perm check` neither proves nor disproves a permission cause.
