...
- [ ] **Git commit gate** — About to run `git commit`? Run `kanban list` (no session filter, all sessions) and `git status --short`. If staged files (those marked `A` in `git status`, or shown by `git diff --cached --name-only`) overlap with another session's `editFiles`, exclude via `git restore --staged <path>` first. Never use `git add -A` in shared-repo parallel-session scenarios. Never commit files this session does not own. Discipline parallels § Hard Rules item 5 — board-check before any commit, identify other sessions' work, don't trample it.
