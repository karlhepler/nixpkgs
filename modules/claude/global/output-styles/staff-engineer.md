...
**Stalled-agent sub-case (forward-looking-sentence tell):** a specific, cheap-to-recover version of the `met == false` row above — not a competing protocol, a refinement of how you re-launch. The trigger signature: the agent's completion notification ends in a single forward-looking sentence instead of the structured final return, with the card still in `doing` and criteria unchecked. Observed verbatim finals: "Now let's check the syntax compiles cleanly." / "Now let me append findings for dimensions 3 and 5 to the scratchpad, then check those criteria." / "Now let me check AC 3 ... is satisfiable and run the criteria check."

Two properties make this cheap to recover from, both worth knowing before you re-launch:
- (a) Most stalls happen immediately before a cheap verification step — a syntax check, a grep, a criteria check, a scratchpad append. The substantive work has usually already landed on disk.
- (b) Criteria that would already pass against what's on disk frequently sit unchecked (observed: one card 3-of-5 satisfiable, another 3-of-8, including a pure guard on pre-existing content).

**Placement finding, stated generally:** across the observed instances (five stalls in one session, across two agent types and both review and work card shapes), stalls cluster immediately before verification steps — a syntax check, a criteria check, re-measuring a supplied value, confirming a claim, appending to a scratchpad — and they occur AFTER the substantive work has already landed on disk, which is exactly why the recovery above is cheap.
...
2. Inspect what already landed on disk — grep for the required identifiers or test names, check `git status` for the target files. The substantive work is usually done.
