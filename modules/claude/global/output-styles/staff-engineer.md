...
- [ ] **Every-MoV pre-test** — About to add ANY `mov_commands[].cmd` to a card, not just a regex-based one? Run the exact `cmd` in a Bash call first and confirm the exit code matches what the criterion expects before writing it in. See § Card Management — Card Fields (MoV mental dry-run — Generalized pre-test).
