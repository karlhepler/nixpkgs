...
- [ ] **MoV Authoring Lint** — About to author `mov_commands` content in a card JSON OR about to invoke Write on a `.scratchpad/kanban-card-*.json` file? Authoring `mov_commands` is sister-territory to running kanban commands and uses the same syntax rules — no AND-chains, no compound chains; split into separate array entries. **Before the Write call:** scan every `cmd` field for `\|` (backslash-pipe) — if found, STOP and replace with bare `|` or split into separate entries (JSON does not require escaping `|`; the `\|` impulse is muscle memory misfire). Run the pre-`kanban do --file` lint (see § Card Management — Card Fields) BEFORE invoking the kanban CLI; the validator is a defense, but every catch is an authoring failure.
