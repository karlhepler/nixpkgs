...
- [ ] **MoV Authoring Lint** — About to author `mov_commands` content in a card JSON, about to invoke Write on a `.scratchpad/kanban-card-*.json` file, OR about to author a `--mov-cmd` value for a mid-flight `kanban criteria add` call? This lint applies equally to criteria authored mid-flight via `kanban criteria add --mov-cmd`, not only at card creation — the same authoring failure can happen in the fast, improvised "just add one more check" moment. Authoring `mov_commands` is sister-territory to running kanban commands and uses the same syntax rules — no AND-chains, no compound chains; split into separate array entries. **Before the Write call (or the `kanban criteria add` invocation):** scan every `cmd` field for `\|` (backslash-pipe) — if found, STOP and replace with bare `|` or split into separate entries (JSON does not require escaping `|`; the `\|` impulse is muscle memory misfire). Run the pre-authoring MoV lint (see § Card Management — Card Fields) BEFORE invoking the kanban CLI (equally for a `kanban criteria add --mov-cmd` value, not only a `kanban do --file` card); the validator is a defense, but every catch is an authoring failure.
...
- [ ] **Backslash-pipe self-test (mov_commands):** Did this response author any `mov_commands[].cmd` fields? If yes: does any `cmd` contain `\|`? Answer yes or no. A backslash-pipe is a LITERAL pipe in rg's regex engine — use a bare `|` for alternation, or split into separate `mov_commands` entries. Full banned-pattern list: § Card Management — Card Fields (Write-tool-time reflex).
