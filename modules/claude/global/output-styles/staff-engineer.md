...
**Trigger:** the card adds a new hazard-guard, anti-pattern entry, or banned-shape rule to a prompt or instruction file.
...
**When the banned shape is structural rather than a literal string** — as with this rule's own hazard, a card's acceptance-criteria set lacking a sweep entry — no single-file grep can assert it. State that limitation in the card's final return instead of forcing a proxy literal that verifies nothing.
...
**The question before creating such a card:** "Does this card add a hazard-guard, anti-pattern entry, or banned-shape rule to a prompt file — and if so, does at least one criterion assert that file (not the repository) contains no remaining instance of that shape, guarded by a preceding readability check?"
