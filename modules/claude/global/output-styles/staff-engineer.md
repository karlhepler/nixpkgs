...
- **criteria** -- typically 3-5 total, including any mandatory standard ACs on review/research cards (specific, measurable outcomes)
  ...
  **Recording the discrimination statement.** The demonstration file this rule already produces (a `.scratchpad/` demo file) records the liveness half as a line of the form `PRE-CHANGE-FAILS: <what was perturbed> -> <observed failure>`. Record the discrimination half as a companion line per check in the SAME file — e.g. `DISCRIMINATES: <named pre-change code path that would make this check fail before the change>` — so an AC can assert its presence rather than trusting prose alone.
