...
**Debugging escalation:** When normal debugging has failed after 2-3 rounds — fixes cause new breakages (hydra pattern), progress stalls despite targeted attempts, or the team is cycling without convergence — suggest `/debugger`. This is NOT an exception skill; it runs as a standard background sub-agent via Task tool. Suggest escalation, confirm with user, then delegate with full bug context (error messages, what's been tried, reproduction steps). **Always use `model: opus` for /debugger delegations — no exceptions.** The debugger's protocol requires scientific rigor, full citation of all claims, systematic assumption enumeration, and multi-round ledger maintenance; this demands maximum model capability. When the debugger returns: act on prioritized recommendations first, read the ledger only if recommendations are insufficient, and fire another round if needed (the debugger detects existing ledgers and continues via cross-round reference). **Pre-delegation check:** Before delegating to /debugger, verify `Write(.kanban/scratchpad/**)` is in the project's `permissions.allow`. Without it, ledger writes fail silently and the cross-round reference capability is lost. Add the permission first if it's absent. **When the debugger returns:** relay findings as hypotheses with confidence levels — not as certainties. See § Trust But Verify and the Debugger overconfidence relay anti-pattern in § Critical Anti-Patterns.
...
- **Delegating to /debugger without ledger write permission** -- Without `Write(.kanban/scratchpad/**)` in `permissions.allow`, the debugger's ledger writes silently fail and every round re-derives the same context. Verify this permission exists before every debugger delegation.
