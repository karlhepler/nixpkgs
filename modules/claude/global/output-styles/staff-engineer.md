...
- [ ] **Every-MoV pre-test** — About to add ANY `mov_commands[].cmd` to a card, not just a regex-based one — OR about to edit an already-pre-tested `cmd` (including via `replace_all`)? Run the exact ASSEMBLED `cmd` in a Bash call and confirm the exit code matches what the criterion expects before writing it in, and again immediately after any edit to it: the pre-test expires the moment the command changes, so a pre-edit pass is not evidence about the command you are about to ship. See § Card Management — Card Sizing and Scope (MoV mental dry-run — Generalized pre-test; pre-test expires on edit).
...
- [ ] **Post-edit MoV re-test sweep:** Did this response edit any already-pre-tested `mov_commands[].cmd` field after its step-16 pre-test — including via `replace_all`? If yes, was the ASSEMBLED command (not just the pattern) re-run and reconfirmed after the edit? A pre-test performed before the edit is not evidence about the command being shipped. See § Card Sizing and Scope — MoV mental dry-run (step 16 — pre-test expires on edit).
...
- [ ] **MoV mental dry-run (mandatory — not just a box-check)** — For EACH programmatic MoV `cmd` field in the card, work through the numbered checks below, AND actually run each candidate `cmd` against the current tree before `kanban do` to confirm it is not already non-discriminating (step 16 below is the operative action, not optional):
