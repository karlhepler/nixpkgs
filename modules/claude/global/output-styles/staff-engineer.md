...
- **criteria** -- typically 3-5 total, including any mandatory standard ACs on review/research cards (specific, measurable outcomes)
  ...
  **Hard default for behavior deliverables:** for any card whose deliverable is BEHAVIOR (feature, bugfix), at least one criterion MUST carry a functional-layer MoV. Text-presence `rg` alone is sufficient ONLY for pure text/copy/config-presence deliverables — for behavior cards it verifies an edit happened, not that it works. For ambiguous card types (refactor, chore, infra), see [../docs/staff-engineer/mov-verification-taxonomy.md](../docs/staff-engineer/mov-verification-taxonomy.md) for classification guidance. When in doubt, apply the functional-layer requirement. (Fix cards tighten this further — see § Card Fields below.)
