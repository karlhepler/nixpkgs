...
**JSON authoring corollary:** When writing `mov_commands[].cmd` to a `.scratchpad/kanban-card-*.json` file via the Write tool, use a SINGLE `|` for alternation — no backslash anywhere. The mental trap: JSON authoring usually requires double-escaping, but for rg alternation in `cmd` fields, NO escaping is needed. **JSON does not require escaping `|` — `|` is a regular character in JSON strings.** The impulse to write `\|` comes from regex/shell muscle memory, NOT from JSON syntax requirements. Path: bare `|` in JSON → bare `|` in shell → alternation in rg. The escape form `\\|` in JSON (which decodes to a backslash-pipe in shell, per JSON's `\\` → `\` rule) matches a LITERAL pipe character in rg's default Rust regex engine. Writing `\|` instead is INVALID JSON per RFC 8259 — the card file fails to parse before any validator runs. Only bare `|` (no backslash anywhere) produces alternation.
  ...
  **Pre-`kanban do --file` `mov_commands` lint (mandatory before every CLI invocation):** Mentally scan every `mov_commands[].cmd` field for these banned patterns BEFORE invoking the kanban CLI:
  ...
  - Any MoV `cmd` ending in `| head -N` / `| tail -N` / `| cut -f N` / `| sort` / `| tr` — discards the upstream command's exit code (a pipeline's exit status is its LAST command's, and these filters exit 0 even on empty stdin); use `rg -q` or terminate the pipeline in something that tests content (`| rg -q .`, `test`). Does not apply to filters feeding a `test`/command-substitution assertion (e.g., `wc -l` piped into `test`). See full entry above.
