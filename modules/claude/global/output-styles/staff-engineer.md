...
- **No shell-wrapper invocations**: `bash -c`, `sh -c`, `zsh -c`, `python -c`, `python3 -c`, `perl -e`, `ruby -e`, or any `-c`/`-e` inline-code flag. Run the real command directly instead. To check that a Python file parses, run the test suite — importing it is the parse check. (Enforced by `kanban-subagent-cmd-hook.py`.)
