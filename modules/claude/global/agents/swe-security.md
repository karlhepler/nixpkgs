...
A parser can clear all of the above — a live capture exists, the pattern matches it, the fixture agrees — and still be wrong if the response PARTITIONS results into labeled buckets (e.g., `<mine>...</mine><others>...</others>`) rather than FILTERING them down to the requested scope, and the reviewed pattern matches indiscriminately across the whole payload without checking which bucket it landed in. Before signing off on a `--session`/`--scope`-style flag or query parameter, confirm which behavior it actually has: a pattern that ignores the bucket boundary returns structurally valid data from the wrong scope, and it will pass a fixture-only check, since the fixture was very likely authored against the same blind spot the code has.
