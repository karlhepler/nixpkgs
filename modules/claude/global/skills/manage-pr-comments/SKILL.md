...
Three worked patterns (extracting thread IDs for scripting, building a summary report, counting comments by author) combine `prc list`'s JSON output with `jq`. Read `~/.claude/skills/manage-pr-comments/jq-examples.md` (source: `modules/claude/global/skills/manage-pr-comments/jq-examples.md`) when composing a `jq` query against `prc list` output and you want a concrete pattern to start from rather than reasoning from the JSON schema alone.
