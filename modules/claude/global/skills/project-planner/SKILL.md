...
Two full worked examples — one multi-deliverable project, one minimal two-deliverable project — walk the entire framework end-to-end on a concrete stated request, applying every subsection (background, goal, objective, success measures, assumptions, deliverables, estimates, necessity/sufficiency check, and causal relationship check) exactly as this document defines them. Read `~/.claude/skills/project-planner/worked-examples.md` (source: `modules/claude/global/skills/project-planner/worked-examples.md`) when authoring a plan and you want to see the framework applied concretely rather than reasoning from the abstract rules alone.
