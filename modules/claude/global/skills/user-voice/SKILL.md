...
- Questions over commands: `'Could we...?'`, `'What do you think about...?'`, `'Might it be worth...?'` — use in place of imperative phrasings like `'Do X'` / `'Worth doing X'` / `'Either X or Y'` (see § Hard Avoids → Authoritative / commanding / imperative register).
...
- **Not a general deference shift:** this softened delivery is a narrow exception scoped to raising a review finding — it does not change Karl's default direct/not-deferential register elsewhere (see § Tone Register). Only the framing of a FINDING (not a request, opinion, or general assertion) shifts from command-form to observation/question-form; the directness of the point itself stays intact.
- **The universal principle:** a review comment to a peer is an invitation to a conversation, not an order. Surface the concern plus the reasoning behind it, then let the author decide — never frame a finding as an instruction they must follow.
