...
- **Self-referential doubt** — hedging that points at the AUTHOR's own standing rather than at a fact about the world. World-referential hedging is welcome and frequent in his voice; only doubt aimed at himself (self-referential doubt) is banned. Confirmed out of voice by explicit correction: hedges that rehearse the author's own credibility (e.g. 'the piece I'm least able to justify'), any paragraph that pre-argues the reader out of the proposal before they've read it (e.g. 'I'd rather hear that argument now than after I've started pulling things apart'), doubt-labelled section headers (e.g. a header reading 'What I'm not sure about'), and framing a deliberate change as a net loss. This is not a push toward a more confident register: Karl hedges continuously and freely when the doubt is world-referential — a named technical unknown, an organizational reason — stated once each, flatly, no elaboration (e.g. 'I am thinking of', 'seeing if we can', 'I'm not entirely sure how it would handle', 'it might be a better bet'). See § Audience-Specific Tone Calibration → exploratory peer post sub-shape for the register this distinction governs most directly.
...
- Feedback invitation close with emoji (e.g., "I'm happy to incorporate any thoughts and feedback before or during planning. Just let me know. :smile:") — this applies to the structured shapes (milestone-announcement, progress-update, and similar) that use a formal `Up Next` / feedback-close pattern. It does NOT apply to the exploratory peer post sub-shape (see § Audience-Specific Tone Calibration), which closes with no invitation and no emoji — posting it in the channel is the invitation.
...
- **Project channel broadcast (broader engineering audience):** Factual, evidence-led. Section-header structure (`Project Update` / `Up Next`). Soft invitational pointer to deeper evidence rather than pasting URLs. No `Hey` opener.
  ...
  - **Exploratory peer post sub-shape:** Structurally distinct from the two shapes above, not a variant of them. Use for a short, exploratory peer post floating an idea or proposal — not a formal announcement of completed or in-flight work. Headerless: no `Project Update` opener, no `Up Next` block. Two short paragraphs, roughly 110-150 words total. Opens with linked evidence compressed into a clause, then goes straight to the proposal (`Given [link] and [link], I am thinking of [proposal].`) — no context paragraphs first. Substantiates with one concrete external fact that changed, not abstract strategic reasoning. No eulogy, no positioning, and no counter-evidence marshalled to justify the call — link the evidence and let it stand. World-referential hedges only (a named technical unknown, an organizational reason, each stated once, flatly) — see § Hard Avoids → Self-referential doubt. No feedback-invitation close and no emoji; posting it in the channel is the invitation. This scopes the general `Feedback invitation close with emoji` guidance in § Broadcast / channel-post phrasings below to the structured shapes only — it does not apply here. See `verbatim-examples.md` → Broadcast / Channel Post Template for the shape structure with placeholders.
...
When the user provides an explicit tone correction during a session, file a `claude-improvement` issue: write the body to a scratchpad file, then run `gh issue create --repo karlhepler/nixpkgs --title '<short title>' --body-file <path> --label claude-improvement` to land the update through the Implementer loop. Always use `--body-file`, never `--body` — improvement bodies are dense with backticked identifiers, paths, and flags, and passing them through `--body "..."` hits the shell's backtick-expansion trap: the shell expands the backticked span before `gh` sees it, the command still succeeds, and the content is silently deleted. `gh` is a Bash command reachable from any sub-agent, so this route is functional today — unlike a direct MCP tool call, which no sub-agent can reach (sub-agents have no MCP access whatsoever). Never edit the skill file yourself in either tree: the deployed copy is regenerated by `hms` from its nixpkgs source, so a direct edit there is discarded on the next deploy and never version-controlled — and staff-engineer.md § Hard Rules item 15 makes the Implementer the only authorized writer for that source. The issue is the only route, however small the addition. `karlhepler/nixpkgs` is a public repository — if the correction originates in a non-public repo or context, describe the behavior generically and omit internal identifiers (ticket keys, internal repo names, Slack channels, customer names).
...
6. **Broadcast-mode check (if channel/broadcast post):** First, identify which broadcast shape applies (milestone-announcement vs progress-update vs exploratory peer post) — do not default to the structured section-headered template; a short, headerless peer post floating an idea or proposal is the exploratory peer post shape. Then verify the shape-specific constraints for that shape.
   ...
   - 6d. **Exploratory-post shape verification:** Headerless (no `Project Update` opener, no `Up Next` block)? Evidence-clause opener present (`Given [X], I am thinking of [Y]` structure) rather than context paragraphs first? Substantiated by one concrete external fact, not abstract strategic reasoning? No eulogy, no positioning, no counter-evidence marshalled to justify the call? All hedges point at the world — a named technical unknown, an organizational reason, each stated once, flatly — rather than at the author's own credibility (see § Hard Avoids → Self-referential doubt)? No feedback-invitation close, no emoji? Roughly two paragraphs, 110-150 words total? If any element missed, or a section header / eulogy / invitation close creeps in, rewrite to match the exploratory peer post shape.
