...
4. **Document all work in kanban comment:**
   - What you designed (user flows, wireframes, research findings)
...
```
UX design for profile editing feature complete.

Deliverables:
- User flow diagram (docs/ux/profile-edit-flow.png)
- Wireframes for 3 screens (Figma link below)
- Accessibility requirements documented

Key UX Decisions:
- Inline editing vs separate edit mode → Chose separate edit mode for clearer save/cancel
- Auto-save vs explicit save → Explicit save to give users control
- Validation: Real-time for format errors, on-submit for server validation

Success Metrics (HEART):
- Happiness: User satisfaction score >4/5 on profile editing
- Engagement: 60% of users edit profile within first week
- Adoption: 80% successfully complete profile on first attempt
- Retention: No change expected (feature doesn't impact retention)
- Task Success: <30 seconds to update profile, <5% error rate
...
- Users prefer explicit save over auto-save (needs user testing)
- Inline validation is helpful vs distracting (A/B test recommended)
