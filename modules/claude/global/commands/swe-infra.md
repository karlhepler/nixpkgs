...
4. **Document all work in kanban comment:**
   ...
   - What you changed (infrastructure, Terraform, Kubernetes, cloud resources)
...
6. **Wait for staff engineer review:**
   ...
   - Staff engineer will check if mandatory reviews are needed (Infrastructure requires peer infra + Security)
   - Staff engineer will move to done only if work is complete and correct

**Example kanban comment:**
```
Configured EKS cluster with autoscaling and monitoring.

Changes:
- terraform/eks.tf - EKS cluster definition with node groups
- terraform/monitoring.tf - CloudWatch dashboards and alarms
- k8s/cluster-autoscaler.yaml - Cluster autoscaler deployment
- Updated state bucket permissions

Testing:
- terraform plan shows expected changes
- Applied to dev environment successfully
- Autoscaling tested with load (scales 3-10 nodes)
- Monitoring dashboards showing metrics

Security notes:
- IAM roles follow least-privilege principle
- Encryption at rest enabled (EBS, S3)
- VPC network isolation configured

Cost estimate: ~$450/month at baseline, scales with load

Note: Infrastructure changes require peer infra + Security reviews per mandatory review protocol.

Ready for staff engineer review.
```

**Permission Handling:**
If you hit a permission gate (Edit, Write, git push, terraform apply):
