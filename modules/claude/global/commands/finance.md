...
4. **Document all work in kanban comment:**
   - What financial analysis you completed
   - Key metrics and findings
   - Recommendations and impact
   - Assumptions and scenarios modeled
...
6. **Wait for staff engineer review:**
   ...
   - Staff engineer will check if mandatory reviews are needed (Financial/billing requires Finance + Security)
   - Staff engineer will move to done only if work is complete and correct

**Example kanban comment:**
```
Unit economics analysis for new pricing model complete.

Deliverables:
- Financial model: models/pricing-model-v2.xlsx
- Scenario analysis (base/upside/downside)
- Recommendation summary

Current State (Flat $99/month):
- MRR: $45K (450 customers)
- Churn: 5%/month
- CAC: $800
- LTV: $1,188 (LTV:CAC = 1.49 - POOR)
- Payback: 8 months

Proposed (Usage-based + $29 base):
Base Case:
- MRR: $78K (projected 650 customers @ avg $120/month)
- Churn: 3%/month (lower friction = better retention)
- CAC: $600 (lower entry point = better conversion)
- LTV: $4,000 (LTV:CAC = 6.67 - EXCELLENT)
- Payback: 5 months

Upside Case (+20% volume):
- MRR: $94K, LTV:CAC = 8.0

Downside Case (-20% volume):
- MRR: $62K, LTV:CAC = 5.3

Key Assumptions:
1. 30% conversion lift from lower entry price ($29 vs $99)
2. Churn improves 2pp (pay for what you use = better value perception)
3. Average usage: $91/month (based on cohort analysis of top 100 customers)
4. CAC decreases 25% (lower barrier to trial)

Sensitivity Analysis:
- Most sensitive to: Average usage per customer (±20% = ±$16K MRR)
- Least sensitive to: Base price ($29 vs $39 = only ±$6K MRR)

Recommendation: PROCEED with usage-based model
- Expected MRR lift: +$33K/month (+73%)
- Expected LTV:CAC improvement: 1.49 → 6.67 (4.5x better)
- Risk: Usage below assumption ($91 → $70 still yields 4.5 LTV:CAC)

Implementation:
1. Grandfather existing customers (6-month notice before migration)
2. A/B test pricing page (2 weeks)
3. Rollout to new customers only (month 1)
4. Migrate existing customers with incentives (months 2-6)

Note: Financial/billing changes require Finance + Security reviews per mandatory review protocol.
