...
## Kanban Text Marker Migration (D#1-D#4)

This section documents the cutover from kanban CLI delegation to text marker processing via the SubagentStop hook.

### What Changed

Sub-agents and AC reviewers now emit text markers instead of calling kanban CLI directly:
- Sub-agents emit: `KANBAN CRITERIA CHECK <N>`, `KANBAN REVIEW`, `KANBAN DONE`
- AC reviewers emit: `KANBAN CRITERIA PASS/FAIL <N>`, `KANBAN DONE`
- The hook (`modules/claude/kanban-ac-hook.py`) parses these markers and calls kanban CLI on their behalf

This decouples agents from kanban CLI permissions and improves feedback quality.

### Emergency Brake: KANBAN_MARKERS Environment Variable

If the marker architecture causes issues, you can immediately disable marker processing without reverting commits:

```bash
# Disable marker processing (fall back to no-op hook behavior)
export KANBAN_MARKERS=0

# Later, re-enable when issues are resolved
export KANBAN_MARKERS=1  # or unset (defaults to enabled)
```

When `KANBAN_MARKERS=0`, the hook skips all marker processing and returns without making blocking decisions, allowing the old CLI-based workflow to operate normally.

### Validation Gate Before Making Cutover Permanent

Before considering this migration permanent, validate:
- **Minimum completion threshold:** 10 completed cards (moved through review to done)
- **Agent diversity:** At least 3 different agent types among the completed cards
- **Quality signal:** AC review loop completes successfully without excessive iteration

Track completion via: `kanban list --output-style=xml --session <session_id>` and review the `done` column across multiple sessions.

### Rollback Path

If unresolved issues arise after the marker cutover:

**Option 1: Immediate (without code changes)**
```bash
export KANBAN_MARKERS=0
```
This immediately disables marker processing. Cards in progress will revert to CLI-based workflow.

**Option 2: Full revert (code rollback)**
Revert the marker architecture commits in this order:
```bash
# Most recent first (reverse chronological)
git revert HEAD            # D#4: Migration cutover (this commit)
git revert HEAD~1          # D#3: Staff prompt cleanup
git revert HEAD~2          # D#2: Delegation templates
git revert HEAD~3          # D#1: Hook engine
```

After reverting, restore old agent instructions and delegation patterns (if they were removed), then:
```bash
git add .
git commit -m "Revert marker architecture migration"
hms  # Redeploy old configuration
```

Each revert is atomic and safe. The hook gracefully handles both marker and CLI-based workflows during intermediate states.

### Monitoring During Transition

**For staff engineer:**
- Watch AC review loop iterations (should stabilize around 1-2 per card)
- Monitor `kanban show` output for proper reviewer_met/agent_met column updates
- Verify cards reach `done` without manual marker-insertion workarounds

**For agents:**
- Emit markers at the end of work (structured, one per line, outside code fences)
- Verify transcript shows marker lines in plain text (not inside ``` ``` code blocks)

**Hook health checks:**
- Marker parsing: Check `.scratchpad/ac-loop-<card>.md` exists during AC review
- CLI execution: Verify `kanban criteria check/pass/fail/review/done` calls appear in card state
- No false blocks: If KANBAN REVIEW is emitted, agent should not receive blocking response
