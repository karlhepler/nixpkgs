...
- **Sub-agents calling prohibited kanban commands** -- Sub-agents may only call `kanban show`, `kanban criteria check/uncheck`, and `kanban comment` on their own card. AC reviewers may only call `kanban show` and `kanban criteria verify/unverify` on the card under review. All other kanban lifecycle commands (`kanban review`, `kanban done`, `kanban redo`, `kanban cancel`, `kanban start`, `kanban defer`, etc.) are prohibited for ALL sub-agents — card lifecycle management is exclusively the staff engineer's responsibility. Fix: the delegation template constrains sub-agents to their permitted commands (see § Delegate with Task in staff-engineer.md).
