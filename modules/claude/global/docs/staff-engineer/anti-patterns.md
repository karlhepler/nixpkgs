...
- **Sub-agents calling kanban CLI instead of emitting markers** -- Sub-agents do not call `kanban criteria check`, `kanban comment`, or `kanban review` directly. They emit text markers (`KANBAN CRITERIA CHECK <N>`, `KANBAN REVIEW`) in their return message; the SubagentStop hook processes them and calls CLI on their behalf. The only permitted CLI call is a single `kanban show` at the END of the agent's work to catch mid-flight AC additions. AC reviewers similarly emit `KANBAN CRITERIA PASS/FAIL <N>` and `KANBAN DONE` markers — never call `kanban criteria pass/fail` or `kanban done` directly. All other kanban lifecycle commands (`kanban redo`, `kanban cancel`, `kanban start`, `kanban defer`, etc.) are prohibited for ALL sub-agents — card lifecycle management is exclusively the staff engineer's responsibility. Fix: the delegation template constrains sub-agents to the marker protocol (see § Delegate with Task in staff-engineer.md).
...
- **Agent finishes without emitting KANBAN REVIEW marker** -- When an agent returns but the hook notification does not arrive (because no `KANBAN REVIEW` marker was emitted), the card stays in doing. The agent completed its substantive work but skipped the marker. The hook cannot chain AC review without the marker. Recovery: check `kanban status <N>` to confirm the card is still in doing, then re-launch the agent for the same card so it can emit the missing markers. The staff engineer must never call `kanban review` directly — the marker is the agent's responsibility.
...
- **Manually completing the sub-agent's marker workflow after a failure** -- When a sub-agent stops without emitting `KANBAN CRITERIA CHECK` or `KANBAN REVIEW` markers, the staff engineer must NOT run `kanban criteria check` or `kanban review` themselves. Those state transitions belong to the agent's marker output — the agent_met column exists to record the agent's own self-assessment. If the agent stopped mid-work without emitting markers, re-launch it (same card, already in doing) so it can emit the markers itself. The correct response is always to re-launch, not to complete the kanban workflow manually. "The substantive work is done and only markers remain" is not an exception. Note: if `kanban show` itself hits a permission gate (the one CLI call sub-agents make), that's a transient platform bug — re-launch immediately without the three-option protocol (see § Permission Gate Recovery in staff-engineer.md).
- **Manually running `perm` to register kanban CLI permissions** -- Never use `perm allow` or `perm always` for kanban commands. Sub-agents only call `kanban show` (one CLI call, at the end of their work); all other kanban state changes use text markers processed by the SubagentStop hook. `Bash(kanban *)` is globally pre-authorized via `~/.claude/settings.json` — no per-card registration is needed. Only non-kanban permissions (e.g., `npm run test`, `git commit`, tool-specific Bash patterns) need manual `perm` registration. Running `perm` for kanban permissions is redundant noise that adds entries without providing any benefit.
...
- **Parsing agent transcript files for findings** — Agent findings belong in the agent's return message, not buried in transcript output. Non-code agents (researcher, lawyer, etc.) include findings directly in their return; the hook and staff engineer read the return message. If you're writing ad-hoc scripts to extract findings from JSON-lines transcript files, the agent failed to surface findings in its return. `kanban redo` and re-delegate.
- Passing AC list in AC reviewer delegation prompt (AC reviewer fetches its own AC via kanban show)
- Calling `kanban criteria check/uncheck` (sub-agent's job via markers) or `kanban criteria pass/fail` (AC reviewer's job via markers)
