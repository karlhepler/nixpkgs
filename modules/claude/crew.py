...
def _split_window_horizontal(session_idx: str, percent: int, cwd: str) -> Optional[str]:
    ...
    `-c <cwd>` sets the new pane's working directory — this is what makes the split
    inherit the source pane's path (mirrors the user's own `-c "#{pane_current_path}"`
    prefix+s keybinding in modules/tmux/default.nix). smithers resolves its target PR
    from the current working directory, so this inheritance is load-bearing: without
    it, bare `smithers` in the new pane would resolve the wrong PR or none at all.
