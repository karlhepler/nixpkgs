...
_CRON_PROMPT = (
    ...
    "  STEP 2: If the only pane listed is your own (the senior-staff window), stop immediately — no output, no crew status, no report. Token waste avoided.\n"
    "  STEP 3: If one or more other Staff panes are present, run `crew status --lines 20` and surface any actionable items (stalled, completed, or errored Staff sessions) per your normal pulse behavior.\n"
    "- Purpose: detect stalled, completed, or errored Staff sessions without waiting for user prompts — but only when there are Staff sessions to monitor.\n"
